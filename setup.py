...
import os
import codecs
import common.tg_analytics as tg_analytic
...
def start_command_handler(message):
    ...
    bot.send_message(message.chat.id, 'Hello, {0}, please choose command from the menu'.format(message.chat.username),
                     reply_markup=markup)
...
def country_command_handler(message):
    cid = message.chat.id
    user_steps[cid] = 1
    bot.send_message(message.chat.id, '{0}, write name of country please'.format(message.chat.username))
...
def hi_command_handler(message):
    with codecs.open('templates/himydear.html', 'r', encoding='UTF-8') as file:
        template = Template(file.read())
        bot.send_message(message.chat.id, template.render(user_name=message.chat.username), parse_mode='HTML')
...
def default_command_handler(message):
    if message.text[:int(os.getenv('PASS_CHAR_COUNT'))] == os.getenv('STAT_KEY'):
        ...
        if 'txt' in st:
            ...
            with codecs.open('%s.txt' % message.chat.id, 'r', encoding='UTF-8') as file:
        ...
        with codecs.open('templates/idunnocommand.html', 'r', encoding='UTF-8') as file:
            template = Template(file.read())
            bot.send_message(message.chat.id, template.render(text_command=message.text), parse_mode='HTML')
